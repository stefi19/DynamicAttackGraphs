...
fn main() {
    ...
    timely::execute_from_args(std::env::args(), |worker| {
        ...
        // Create the dataflow
        let (mut vulnerability_input, mut network_access_input, mut attacker_input) = worker
            .dataflow::<usize, _, _>(|scope| {
                ...
                let (vuln_handle, vulnerability_collection) =
                    scope.new_collection::<(String, String, bool), isize>();

                // Network access: (source_host, destination_host, service_name)
                let (access_handle, network_access_collection) =
                    scope.new_collection::<(String, String, String), isize>();

                // Attacker location: (attacker_id, current_host)
                let (attacker_handle, attacker_location_collection) =
                    scope.new_collection::<(String, String), isize>();
                ...
                let all_code_executions = initial_attacker_positions.iterate(|current_positions| {
                    let network_access_in_scope =
                        network_access_collection.enter(&current_positions.scope());

                    // Get just the host+service pairs that have vulnerabilities
                    let vulnerable_host_service_pairs = vulnerability_collection
                        .enter(&current_positions.scope())
                    ...
                    current_positions
                        ...
                        .join(
                            &network_access_in_scope.map(|(source, destination, service)| {
                                (source, (destination, service))
                            }),
                        )
                        // Result: (source_host, (attacker_id, (destination_host, service)))
                        .map(|(_source, (attacker_id, (destination, service)))| {
                            ((destination.clone(), service), (attacker_id, destination))
                        })
                        ...
                        .map(|((_host, _service), (attacker_id, destination))| {
                            (attacker_id, destination)
                        })
                ...
                let machines_with_root_access = all_code_executions
                    ...
                    .join(
                        &vulnerability_collection
                            .filter(|(_host, _service, grants_root)| *grants_root)
                            .map(|(host, _service, _root)| (host, ())),
                    )
                ...
                all_code_executions
                    .inspect(|change| {
                        ...
                        println!(
                            "  [t={}] {} execCode({}, {})",
                            timestamp, change_type, attacker_id, host_name
                        );
                    })
